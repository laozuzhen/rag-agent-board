...
export function setApiKeyAuthEnabled(enabled?: boolean): void {
  (globalThis as any).__AGENTBOARD_API_KEY_AUTH_ENABLED = enabled;
}

function isApiKeyAuthEnabled(): boolean {
  const override = (globalThis as any).__AGENTBOARD_API_KEY_AUTH_ENABLED;
...
const OPENCLAW_DIR = process.env.AGENTBOARD_OPENCLAW_DIR || pathLib.join(os.homedir(), ".openclaw");

export function setOpenClawDir(dir?: string): void {
  // For testing: override the OpenClaw dir at runtime
  (globalThis as any).__OPENCLAW_DIR = dir;
}

export function setAutoDiscoverOpenClaw(enabled?: boolean): void {
  // For testing: override auto-discovery at runtime
  (globalThis as any).__AUTO_DISCOVER_OPENCLAW = enabled;
}
...
function getOpenClawDir(): string {
  return (globalThis as any).__OPENCLAW_DIR || process.env.AGENTBOARD_OPENCLAW_DIR || OPENCLAW_DIR;
}

function isOpenClawAutoDiscoveryEnabled(): boolean {
  const override = (globalThis as any).__AUTO_DISCOVER_OPENCLAW;
  if (typeof override === "boolean") return override;
  return parseBooleanEnv(process.env.AGENTBOARD_AUTO_DISCOVER_OPENCLAW, true);
}

function readOpenClawAgentNames(openClawDir: string): Map<string, string> {
  const names = new Map<string, string>();
  const fp = pathLib.join(openClawDir, "openclaw.json");
  if (!existsSync(fp)) return names;

  try {
    const config = JSON.parse(readFileSync(fp, "utf-8")) as any;
    ...
    for (const item of list) {
      ...
      if (!id) continue;
      const identityName = typeof item?.identity?.name === "string" ? item.identity.name.trim() : "";
      const explicitName = typeof item?.name === "string" ? item.name.trim() : "";
      names.set(id, identityName || explicitName || id);
    }
  } catch {
    // Ignore malformed config and keep directory-based discovery
  }

  return names;
}

function discoverOpenClawAgents(): Agent[] {
  if (!isOpenClawAutoDiscoveryEnabled()) return [];

  const openClawDir = getOpenClawDir();
  if (!existsSync(openClawDir)) return [];

  const ids = new Set<string>(["main"]);
  const agentsDir = pathLib.join(openClawDir, "agents");
  if (existsSync(agentsDir)) {
    try {
      ...
      // Ignore unreadable agent dir and return best-effort result
    }
  }

  const names = readOpenClawAgentNames(openClawDir);
  return [...ids].sort().map((id): Agent => ({
    id,
...
function mergeAgentLists(registered: Agent[], discovered: Agent[]): Agent[] {
  if (discovered.length === 0) return registered;
  const merged = [...registered];
  const seen = new Set(registered.map(a => a.id));
...
router.get("/agents", (_req: Request, res: Response) => {
  const registered = store.getAgents();
  const discovered = discoverOpenClawAgents();
  res.json(mergeAgentLists(registered, discovered));
});
